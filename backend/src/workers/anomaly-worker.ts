...
export const anomalyWorker = new Worker(
  ...
  async (job: Job) => {
    ...
    try {
      ...
      const readingQuery = `
        ...
        WHERE r.id = ANY($1)
      `;
      const { rows: readings } = await query(readingQuery, [readingIds]);
      if (readings.length === 0) return;

      // Extract unique sensors from this ingested batch
      const sensorIds = [...new Set(readings.map(r => r.sensor_id))];

      // 2. Load Configured Rules for those sensors
      const rulesQuery = `SELECT * FROM rules WHERE sensor_id = ANY($1) AND rule_type IN ('threshold', 'rate_of_change')`;
      const { rows: allRules } = await query(rulesQuery, [sensorIds]);
      ...
      for (const reading of readings) {
        ...
        for (const rule of rulesForSensor) {
          ...
          if (rule.rule_type === 'threshold') {
            const config = rule.config as { min_temp: number, max_temp: number, max_voltage: number, min_voltage: number };
            if (
          ...
          if (rule.rule_type === 'rate_of_change') {
            ...
            // Get past 3 readings for the sensor BEFORE this new timestamp
            const pastQuery = `
              SELECT ${config.metric} as val FROM readings 
              WHERE sensor_id = $1 AND timestamp < $2 
              ORDER BY timestamp DESC LIMIT 3
            `;
            const { rows: history } = await query(pastQuery, [reading.sensor_id, reading.timestamp]);
            if (history.length === 3) {
          ...
          if (triggered) {
            await applyTrigger(rule, reading);
          }
        }
      }
  ...
  { connection, concurrency: 50 } // Handle heavy parallel volume
);

async function applyTrigger(rule: any, reading: any) {
  const anomalyId = uuidv4();
  
  // 1. Verify Suppression Window (Active or Not)
  const isSuppressed = await query(
  ...
  // 2. Insert Anomaly Record (Must occur even if suppressed)
  await query(
  ...
  // 3. Write Alerts & Update State only if NOT suppressed
  if (!suppressed) {
    const alertId = uuidv4();
    await query(
      `INSERT INTO alerts (id, anomaly_id, severity, status) VALUES ($1, $2, $3, 'open')`,
      [alertId, anomalyId, rule.severity]
    );

    // Update sensor state to emit SSE realtime metric efficiently
    ...
    // Push state-change Event via Server Sent Events
